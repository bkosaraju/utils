...
trait SQLTemplateWrapper
   ...
   * @param templateType - template type as such mustache,ssp, scaml etc ..
   ...
   *         }}}
   */
  def getSQLFromTemplate(config : Map[String,String], templateType : String = "mustache"): String = {
    val templateFileLocation =  Files.createTempDirectory("")
    try {
      val templateFile = templateFileLocation + "/sqlFile.template"
